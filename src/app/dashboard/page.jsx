...
export default function DashboardPage() {
  ...
  const members = useMemo(() =>
    users.filter(u => u.role === 'member').sort((a, b) => new Date(b.joinedAt) - new Date(a.joinedAt)),
    [users]
  );
      ...
      <Box sx={{ background: 'linear-gradient(135deg, #1E1B4B 0%, #312E81 60%, #4338CA 100%)', py: 5 }}>
        <Container maxWidth="lg">
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
            ...
            <Button
              component={Link} href="/events/create"
          ...
          {STAT_CARDS.map(s => (
            <Grid item xs={6} md={3} key={s.label}>
              <Paper sx={{ p: 2.5 }} elevation={0}>
                ...
                <Typography variant="h4" fontWeight={800} sx={{ fontFamily: '"Syne", sans-serif', lineHeight: 1 }}>
                  {s.value}
                </Typography>
                      ...
                      <TableRow key={event.id} hover sx={{ '&:last-child td': { border: 0 } }}>
                        <TableCell sx={{ maxWidth: 280 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                            <Box sx={{
                              width: 8, height: 40, borderRadius: 1,
                              background: event.colorTag || '#4F46E5',
                              flexShrink: 0,
                            }} />
                        ...
                        <TableCell>
                          <Chip
                            ...
                            size="small"
                            sx={{
                              background: `${event.colorTag || '#4F46E5'}15`,
                        ...
                        <TableCell sx={{ minWidth: 140 }}>
                          <Typography fontSize="0.8rem" fontWeight={600} mb={0.5}>
                          ...
                          <LinearProgress
                            ...
                            sx={{
                              ...
                              '& .MuiLinearProgress-bar': {
                                borderRadius: 2,
                                background: fill >= 90 ? '#EF4444' : event.colorTag || '#4F46E5',
                              },
                            }}
                          />
                        </TableCell>
                        <TableCell>
                          <Chip
                            ...
                            size="small"
                            sx={{
                              background: upcoming ? '#D1FAE5' : '#F3F4F6',
                              color: upcoming ? '#047857' : '#6B7280',
                              fontWeight: 700, fontSize: '0.7rem',
                            }}
                          />
                        </TableCell>
                        <TableCell align="right">
                          <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end' }}>
                            <Tooltip title="View">
                              <IconButton size="small" component={Link} href={`/events/${event.id}`}>
                                <VisibilityIcon sx={{ fontSize: 18 }} />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="Edit">
                              <IconButton size="small" component={Link} href={`/events/create?edit=${event.id}`}>
                                <EditIcon sx={{ fontSize: 18, color: 'primary.main' }} />
                              </IconButton>
                            </Tooltip>
                            <Tooltip title="Delete">
                              <IconButton size="small" onClick={() => setDeleteTarget(event.id)}>
                  ...
                  {events.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={6} sx={{ textAlign: 'center', py: 6, color: 'text.secondary' }}>
                        No events yet.{' '}
                        <Box component={Link} href="/events/create" sx={{ color: 'primary.main', fontWeight: 700 }}>
              ...
              <Table>
                ...
                <TableBody>
                  {[...users].sort((a, b) => a.role === 'admin' ? -1 : 1).map(user => {
                    const eventsGoing = events.filter(e =>
                    ...
                    return (
                      <TableRow key={user.id} hover sx={{ '&:last-child td': { border: 0 } }}>
                        ...
                        <TableCell>
                          <Typography fontSize="0.875rem" color="text.secondary">{user.email}</Typography>
                        </TableCell>
                        <TableCell>
                          <Chip
      ...
      <Dialog open={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)}
        ...
        <DialogContent>
          <Typography color="text.secondary">
            This action is permanent and all RSVPs will be lost.
          </Typography>
        </DialogContent>
