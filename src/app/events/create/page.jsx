        ...
        <Container maxWidth="lg">
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{
              ...
              background: `linear-gradient(135deg, ${form.colorTag}, ${form.colorTag}99)`,
            ...
            <Grid item xs={12} md={8}>
              ...
              <Paper sx={{ p: 3, mb: 3 }} elevation={0}>
                ...
                <TextField
                  ...
                  fullWidth value={form.location}
                  onChange={set('location')}
                  placeholder="e.g. WeWork Downtown Nashville, or Online – Zoom"
                  sx={{ mb: 2 }}
                  helperText='Type "Online" or "Zoom" to mark as a virtual event'
                />
                <TextField
                  label="Street Address"
                  fullWidth value={form.address}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      helperText="Max number of attendees"
                    />
                  </Grid>
                  <Grid item xs={12}>
                    <TextField
                      label="Tags"
                      fullWidth value={form.tags}
                      onChange={set('tags')}
            ...
            <Grid item xs={12} md={4}>
              ...
              <Paper sx={{ p: 3, mb: 3 }} elevation={0}>
                    ...
                    <Tooltip key={c.value} title={c.label}>
                      <Box
                        ...
                        sx={{
                          ...
                          background: c.value,
                          border: form.colorTag === c.value ? `3px solid ${c.value}` : '3px solid transparent',
                          outline: form.colorTag === c.value ? `2px solid #fff, 4px solid ${c.value}` : 'none',
                ...
                <Box sx={{ mt: 2.5, p: 1.5, borderRadius: 2, background: `${form.colorTag}12`, border: `1px solid ${form.colorTag}30` }}>
                  ...
                  <Typography fontSize="0.8rem" fontWeight={700} color={form.colorTag}>
                    {form.category}
                  </Typography>
                  <Typography fontSize="0.85rem" fontWeight={600} mt={0.5} color="text.primary" noWrap>
              ...
              <Paper sx={{ p: 3 }} elevation={0}>
                ...
                <Typography variant="caption" color="text.secondary" display="block" mb={2.5}>
                  {isEdit
                    ...
                    : 'Your event will be visible to all members once published.'}
                </Typography>
                <Button
                  type="submit"
                  variant="contained"
                ...
                <Button
                  component={Link}
                  href={isEdit ? `/events/${editId}` : '/events'}
                  variant="outlined"
                  fullWidth
                  disabled={saving}
                >
...
// Need to add Tooltip import
function Tooltip({ children, title }) {
  return (
    <Box title={title} sx={{ display: 'inline-block' }}>
      {children}
    </Box>
  );
}
