...
export default function EventDetailPage() {
  ...
  if (!event) {
    return (
      <Container maxWidth="sm" sx={{ py: 10, textAlign: 'center' }}>
        ...
        <Button component={Link} href="/events" startIcon={<ArrowBackIcon />} variant="contained">
          Back to Events
        </Button>
      </Container>
    );
  ...
  return (
    <Box>
      ...
      <Box sx={{
        height: 8,
        background: `linear-gradient(90deg, ${event.colorTag || '#4F46E5'}, ${event.colorTag || '#4F46E5'}80)`,
      }} />
          ...
          <Grid item xs={12} md={8}>
            ...
            <Box sx={{ display: 'flex', gap: 1, mb: 2, flexWrap: 'wrap', alignItems: 'center' }}>
              <Chip
                ...
                sx={{
                  background: `${event.colorTag || '#4F46E5'}18`,
                  color: event.colorTag || '#4F46E5',
                  fontWeight: 700,
                }}
              />
              {event.online && (
            ...
            <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2, mb: 3 }}>
              <Typography variant="h3" sx={{ fontSize: { xs: '1.6rem', md: '2.2rem' }, lineHeight: 1.2, flex: 1 }}>
                {event.title}
              </Typography>
              {isAdmin && (
                <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                  <Tooltip title="Edit event">
                    <IconButton
                      ...
                      sx={{ background: 'rgba(79,70,229,0.08)', '&:hover': { background: 'rgba(79,70,229,0.16)' } }}
                    >
                  ...
                  <Tooltip title="Delete event">
                    <IconButton
                      ...
                      sx={{ background: 'rgba(239,68,68,0.08)', '&:hover': { background: 'rgba(239,68,68,0.16)' } }}
                    >
                ...
                {[
                  ...
                  <Grid item xs={12} sm={6} key={item.label}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5 }}>
                      <Box sx={{ color: event.colorTag || 'primary.main', mt: 0.2, '& svg': { fontSize: 20 } }}>
                        {item.icon}
                      </Box>
                      <Box>
                        <Typography variant="caption" color="text.secondary" fontWeight={600} display="block" sx={{ textTransform: 'uppercase', letterSpacing: '0.06em', fontSize: '0.7rem' }}>
                          {item.label}
                        </Typography>
            ...
            <Typography
              ...
              sx={{ lineHeight: 1.8, whiteSpace: 'pre-wrap', mb: 3 }}
            >
              {event.description}
            </Typography>
            ...
            {event.tags?.length > 0 && (
              <Box sx={{ mb: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  {event.tags.map(tag => (
                    <Chip key={tag} label={`#${tag}`} variant="outlined" size="small" sx={{ borderRadius: 1, fontWeight: 600 }} />
                  ))}
                </Box>
              </Box>
            )}
            ...
            <Typography variant="h6" fontWeight={700} mb={2}>
              Attendees ({goingAttendees.length}/{event.capacity})
            </Typography>
          ...
          <Grid item xs={12} md={4}>
            <Box sx={{ position: 'sticky', top: 88 }}>
              ...
              <Paper sx={{ p: 3, mb: 3 }} elevation={0}>
                <Typography variant="h6" fontWeight={700} mb={2}>
                  {isPast ? 'This event has ended' : 'Reserve your spot'}
                </Typography>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.75 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      ...
                      <Typography variant="caption" fontWeight={600} color="text.secondary">
                        {goingAttendees.length} / {event.capacity} going
                      </Typography>
                    </Box>
                    <Typography variant="caption" fontWeight={700}
                      color={isFull ? 'error.main' : spotsLeft <= 5 ? 'warning.main' : 'success.main'}>
                  ...
                  <LinearProgress
                    ...
                    sx={{
                      ...
                      '& .MuiLinearProgress-bar': {
                        borderRadius: 3,
                        background: isFull ? '#EF4444' : `${event.colorTag || '#4F46E5'}`,
                      },
                    }}
                  />
                </Box>
                ...
                {!currentUser ? (
                  ...
                  <Box sx={{ display: 'flex', gap: 1.5, flexDirection: 'column' }}>
                    <Button
                      variant={myRsvp?.status === 'going' ? 'contained' : 'outlined'}
                      color="success"
                      fullWidth
                      size="large"
                      disabled={isFull && myRsvp?.status !== 'going'}
                    ...
                    <Button
                      variant={myRsvp?.status === 'declined' ? 'contained' : 'outlined'}
                      color="inherit"
                      fullWidth
                    ...
                    <Box>
                      ...
                      {organizer.role === 'admin' && (
                        <Chip label="Admin" size="small" sx={{
                          ...
                          color: '#fff', fontWeight: 700, fontSize: '0.7rem', height: 20,
                        }} />
                      )}
                    </Box>
                  </Box>
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)} PaperProps={{ sx: { borderRadius: 3, p: 1 } }}>
        ...
        <DialogContent>
          <Typography color="text.secondary">
            This action cannot be undone. All RSVPs will be lost.
          </Typography>
        </DialogContent>
