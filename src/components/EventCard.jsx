...
const goingCount = (event) => (event.attendees || []).filter(a => a.status === 'going').length;

export default function EventCard({ event, myRsvp, compact = false }) {
  ...
  return (
    <Link href={`/events/${event.id}`} className="event-card-link" style={{ display: 'block' }}>
      <Card sx={{ height: '100%', position: 'relative', overflow: 'hidden', opacity: past ? 0.75 : 1 }}>
        ...
        <Box sx={{
          height: 5,
          background: `linear-gradient(90deg, ${event.colorTag || '#4F46E5'}, ${event.colorTag || '#7C3AED'}99)`,
        }} />
        ...
        <CardContent sx={{ p: compact ? 2 : 2.5 }}>
          ...
          <Typography variant={compact ? 'subtitle1' : 'h6'}
            fontWeight={700}
            sx={{ mb: 1.5, lineHeight: 1.3, color: 'text.primary',
